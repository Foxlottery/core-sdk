import { Currency, CurrencyAmount, Ether, NativeCurrency, Percent, Erc20Currency } from '../../src/entities'
...
describe('CurrencyAmount', () => {
  ...
  it('token amount can be max uint256', () => {
    const amount = CurrencyAmount.fromRawAmount(new Erc20Currency(1, ADDRESS_ONE, 18), MaxUint256)
    expect(amount.quotient).toEqual(MaxUint256)
  })
  it('token amount cannot exceed max uint256', () => {
    expect(() =>
      CurrencyAmount.fromRawAmount(new Erc20Currency(1, ADDRESS_ONE, 18), JSBI.add(MaxUint256, JSBI.BigInt(1)))
    ).toThrow('AMOUNT')
  })
  it('token amount quotient cannot exceed max uint256', () => {
    expect(() =>
      CurrencyAmount.fromFractionalAmount(
        new Erc20Currency(1, ADDRESS_ONE, 18),
  ...
  describe('#toFixed', () => {
    it('throws for decimals > currency.decimals', () => {
      const token = new Erc20Currency(1, ADDRESS_ONE, 0)
  ...
  describe('#serialize', () => {
    it('renders string with address and amount for Erc20Currency', () => {
      const token = new Erc20Currency(1, ADDRESS_ONE, 3, 'MOONBEAM')
      const amount = CurrencyAmount.fromRawAmount(token, 123456)
    ...
    it('renders string with address and amount for NativeCurrency', () => {
      class MoonBeam extends NativeCurrency {
        ...
        get wrapped(): Erc20Currency {
          return new Erc20Currency(ChainId.ETHEREUM, ADDRESS_ONE, 18)
        }
      }
